...
{
    /*
        The function takes three input parameters: the original vector arr, 
        and the two sub-vectors left and right that are to be merged.

        It initializes three index variables: i for left, j for right, and k for arr. These variables 
        ...
        it means that the element from left should be placed in arr[k] because it is smaller or equal. 
        So, the element is copied from left[i] to arr[k], and both i and k are incremented.

        If the element at left[i] is greater than the element at right[j], 
        it means that the element from right should be placed in arr[k] because it is smaller. 
        So, the element is copied from right[j] to arr[k], and both j and k are incremented.

        After copying an element to arr, the loop continues to the next iteration, 
        ...
        The function then enters two separate loops to copy any remaining elements to arr.

        The first loop copies any remaining elements from left to arr, starting from index i 
        and continuing until the end of left. It increments both i and k after each copy.

        The second loop copies any remaining elements from right to arr, starting from index j 
        and continuing until the end of right. It increments both j and k after each copy.

        Finally, the merged and sorted elements are now stored in the original vector arr, 
...
void merge_sort(std::vector<int>&vec) 
{
    ...
    */
    ...
    /* 
        ...
        left is a copy of the elements from the beginning of arr up to the middle index 
        while right is a copy of the elements from the middle index to the end of arr.
    */
